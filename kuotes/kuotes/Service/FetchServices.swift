...
class FetchServices {
    ...
    fileprivate func fetchJSON<T: Decodable>(from endpoint: String) async throws
        -> T
    {
        ...
        req.setValue(
            "Basic \(Data(loginString.utf8).base64EncodedString())",
    ...
    fileprivate func propfind(from endpoint: String, depth: String = "5")
        async throws -> String
    {
        guard let url = makeURL(from: endpoint) else { throw URLError(.badURL) }
        var req = URLRequest(url: url)
